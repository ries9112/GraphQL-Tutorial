...
When dealing with APIs the data will often be in the **JSON** format, which is the output we showed above, and usually needs to be manipulated into a format the user is used to working with. We can use the `jsonlite` [[@R-jsonlite]](<https://arxiv.org/abs/1403.2805>) package to easily convert the data from the JSON format to being a dataframe which is the standard way of storing tabular data in R by using the `fromJSON()` function:
...
```{r hitbtc_exercise_data_pull, exercise=TRUE}
